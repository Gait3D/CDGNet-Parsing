...
LABELSNEW = ['Background', 'Head', 'Torso', 
            'Left-arm', 'Right-arm', 
...
def compute_mean_ioU_new(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val'):
    confusion_matrix = np.zeros((num_classes, num_classes))

    for i, im_name in enumerate(os.listdir(datadir)):
    ...
    for i, (label, iou) in enumerate(zip(LABELSNEW, IoU_array)):
